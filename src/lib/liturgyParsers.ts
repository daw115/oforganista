/**
 * Liturgy parsers â ported from ofiarowanie-app
...
function splitSongTitleInlineNote(raw: string) {
  ...
  const match = text.match(/^(.*?)([Ww]\s*nawi[aÄ]zaniu do\s+.+)$/i);
...
async function fetchHtml(url: string, force = false): Promise<string> {
  ...
  const score = (txt: string) => {
    ...
    const mojibake = (txt.match(/[ÃÃÃ]/g) || []).length;
    const polish = (txt.match(/[ÄÄÄÅÅÃ³ÅÅºÅ¼ÄÄÄÅÅÃÅÅ¹Å»]/g) || []).length;
...
async function fetchHtmlFromSources(url: string, force = false): Promise<string> {
  ...
  throw lastError instanceof Error ? lastError : new Error('Nie udaÅo siÄ pobraÄ danych');
}
...
// ââââ SONGS (musicamsacram.pl) âââââââââââââââââââââââââââââââ
...
function parseSongsHtml(html: string, sourceUrl: string): SongsData {
  ...
  const title = cleanText(doc.querySelector('title')?.textContent || 'Propozycje ÅpiewÃ³w');
  ...
  const parseSectionItems = (root: ParentNode): SongSection[] => {
    ...
    for (const header of sectionHeaders) {
      ...
      if (!name || !/^Åpiew /i.test(name)) continue;
  ...
  for (let pi = 0; pi < panes.length; pi++) {
    ...
    if (!setName) {
      const paneHeader = Array.from(pane.querySelectorAll('h2.page-header'))
        .find((h) => !/^Åpiew /i.test(cleanText(h.textContent || '')));
  ...
  console.log(`[SongsParser] Total sets: ${sets.length} â ${sets.map(s => s.name).join(' | ')}`);
  return { title, sets, sourceUrl };
}

// ââââ READINGS (brewiarz.pl) âââââââââââââââââââââââââââââââââ
...
function extractBrewiarzSubPageLinks(html: string, baseUrl: string): { name: string; url: string }[] {
  const doc = new DOMParser().parseFromString(html, 'text/html');
  const links = Array.from(doc.querySelectorAll('a[href]')) as HTMLAnchorElement[];
  const subPages: { name: string; url: string }[] = [];
  for (const link of links) {
    const href = link.getAttribute('href') || '';
    if (/czyt2\.php3/i.test(href)) {
      const name = cleanText(link.textContent || '');
      if (name) {
        try {
          const abs = new URL(href, baseUrl).toString();
          subPages.push({ name, url: abs });
        } catch { /* skip */ }
      }
    }
  }
...
function extractBrewiarzReadingsHtml(html: string): string {
  ...
  const shouldDropText = (text: string) => {
    ...
    return t.includes('w wersji premium')
      ...
      || t.includes('jeÅli strona nie wyÅwietla siÄ poprawnie');
  };

  // Extract reference and subtitle from section's first table text
  const knownLabels = [
    'PIERWSZE CZYTANIE', 'DRUGIE CZYTANIE', 'CZYTANIE',
    'PSALM RESPONSORYJNY', 'ÅPIEW PRZED EWANGELIÄ', 'AKLAMACJA', 'EWANGELIA',
  ];
  ...
  const extractHeaderInfo = (sectionTables: Element[]): { ref: string; subtitle: string } => {
    ...
    for (const row of rows) {
      ...
      for (const cell of cells) {
        ...
        if (!subtitle) {
          for (const label of knownLabels) {
            if (txt.startsWith(label) && txt.length > label.length + 2) {
              ...
              // Remove "â komentarze" prefix if present
              rest = rest.replace(/^[ââ-]\s*komentarze\s*/i, '').trim();
        ...
        if (!ref && /^(\d\s+)?[A-ZÅ»Å¹ÄÅÅÃÄÄ][a-zÅ¼ÅºÄÄÅÅÃ³ÄÅ]{0,4}\s+\d/.test(txt)) {
          ref = txt;
          continue;
        }
      }
  ...
  for (let i = 0; i < positions.length; i++) {
    ...
    const cleaned = sectionTables
      .map((table, tableIdx) => {
        ...
        if (!text || text.includes('antyfona na wej') || text.includes('kolekta')
          ...
          || text.includes('wprowadzenie do modlitwy paÅskiej') || text.includes('odmawia siÄ')) {
          return '';
        }
    ...
    if (positions[i].name === 'ewang' && cleaned.length > 0) {
      const cutAt = Math.max(
        cleaned.join('<br>').toLowerCase().lastIndexOf('oto sÅowo paÅskie'),
...
export async function fetchReadings(date: Date): Promise<ReadingsData> {
  ...
  for (const opt of options) {
    try {
      ...
      let contentHtml = extractBrewiarzReadingsHtml(html);

      // Fallback: if no readings found, check for sub-page links (e.g. Palm Sunday)
      if (!contentHtml) {
        const subPages = extractBrewiarzSubPageLinks(html, opt.url);
        if (subPages.length > 0) {
          console.log(`[Readings] No inline readings for "${opt.name}", found ${subPages.length} sub-page(s): ${subPages.map(s => s.name).join(' | ')}`);
          for (const sub of subPages) {
            try {
              const subHtml = await fetchBrewiarzHtml(sub.url);
              const subContent = extractBrewiarzReadingsHtml(subHtml);
              if (subContent) {
                console.log(`[Readings] Sub-page "${sub.name}": content ${subContent.length} chars`);
                readingsOptions.push({ name: sub.name, url: sub.url, contentHtml: subContent });
              }
            } catch (subErr) {
              console.error(`[Readings] Failed to fetch sub-page "${sub.name}":`, subErr);
            }
          }
          continue;
        }
      ...
      readingsOptions.push({ name: opt.name, url: opt.url, contentHtml: '<div>Nie udaÅo siÄ pobraÄ czytaÅ.</div>' });
    }
  }
...
// ââââ CALENDAR (brewiarz.pl show.php3) âââââââââââââââââââââââ
...
function extractBrewiarzCalendarHtml(html: string, sourceUrl: string): CalendarData {
  ...
  if (!contentRoot) {
    return { title: dayTitle, contentHtml: '<div>Brak treÅci kartki dla tej daty.</div>', sourceUrl };
  }
  ...
  const shouldDropPremiumLine = (txt: string) => {
    ...
    return (
      ...
      t.includes('dostÄpie premium') ||
      t.includes('inne oficja i peÅna treÅÄ') ||
      t.includes('przejdÅº do wersji premium') ||
      t.includes('dostÄp do wersji premium') ||
